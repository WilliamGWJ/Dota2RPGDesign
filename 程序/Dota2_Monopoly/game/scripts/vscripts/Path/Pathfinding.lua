...
function pathFinding:getPath(player, stepsCount)
    ...
    --存储走过的路
    if #pathNew > 2 then
        DOTA2XGameMode.vPATHOld[player:GetPlayerID()] = {
            row = pathNew[#pathNew - 2].row,
            col = pathNew[#pathNew - 2].col
        }
    end
    return pathNew
end
    ...
    GameRules:GetGameModeEntity():SetContextThink(
        ...
        function()
            ...
            if #(DOTA2XGameMode.vPaths[playerid]) ~= 0 then
                ...
                local x, y, z =
                    pathFinding:GetPosByRowAndCol(
                    DOTA2XGameMode.vPaths[playerid][1].row,
                    DOTA2XGameMode.vPaths[playerid][1].col
                )

...
function BuildingHelper:SetFireGameEvents(bFireGameEvents)
    if bFireGameEvents then
        FIRE_GAME_EVENTS = true
    end
end
...
function BuildingHelper:AddBuildingToGrid(vPoint, nSize, vOwnersHero)
    ...
    if tableContains(BH_UNITS, vOwnersHero) then
        ...
        print(
            "You haven't added the owner as a unit. No pathing map will be generated, and the owner may get stuck after building the building."
        )
    end
...
function BuildingHelper:AddBuilding(building)
    ...
    BuildingHelper:CreateTimer(
        building.BuildingTimerName,
        {
            ...
            callback = function(sheeptag, args)
                ...
                if IsValidEntity(building) then
                    if building.bUpdatingHealth then
                        if
                            building:GetHealth() < building.nMaxHealth and
                                GameRules:GetGameTime() <= building.fTimeBuildingCompleted
                         then
                    ...
                    if building.bFireEnabled then
                        ...
                        elseif
                            building:GetHealth() > building:GetMaxHealth() / 2 and
                                building:HasModifier(building.fireEffect)
                         then

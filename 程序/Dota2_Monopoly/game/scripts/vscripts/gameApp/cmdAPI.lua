...
function CosmeticLib:Init()
    if not CosmeticLib.bHasInitialized then
        ...
        Convars:RegisterCommand(
            'print_item_sets_for_player',
            function(cmd, player_id)
                return CosmeticLib:PrintSetsForHero(PlayerResource:GetPlayer(tonumber(player_id)))
            end,
            'Print set item for hero',
            FCVAR_CHEAT
        )

        Convars:RegisterCommand(
            'print_items_from_player',
            function(cmd, player_id)
                return CosmeticLib:PrintItemsFromPlayer(PlayerResource:GetPlayer(tonumber(player_id)))
            end,
            'Print items currently equipped to assigned hero',
            FCVAR_CHEAT
        )

        Convars:RegisterCommand(
            'print_items_for_slot_from_player',
            function(cmd, player_id, slot_name)
                return CosmeticLib:PrintItemsForSlotFromPlayer(PlayerResource:GetPlayer(tonumber(player_id)), slot_name)
            end,
            'Print items available for certain slot in hero',
            FCVAR_CHEAT
        )

        Convars:RegisterCommand(
            'equip_item_set_for_player',
            function(cmd, player_id, set_id)
                local hero = PlayerResource:GetPlayer(tonumber(player_id)):GetAssignedHero()

                return CosmeticLib:EquipHeroSet(hero, set_id)
            end,
            'Equip set item for hero',
            FCVAR_CHEAT
        )

        Convars:RegisterCommand(
            'replace_item_by_slot',
            function(cmd, player_id, slot_name, item_id)
                local hero = PlayerResource:GetPlayer(tonumber(player_id)):GetAssignedHero()

                return CosmeticLib:ReplaceWithSlotName(hero, slot_name, item_id)
            end,
            'Replace item by slot name',
            FCVAR_CHEAT
        )

        Convars:RegisterCommand(
            'replace_item_by_id',
            function(cmd, player_id, old_item_id, new_item_id)
                local hero = PlayerResource:GetPlayer(tonumber(player_id)):GetAssignedHero()
        ...
        Convars:RegisterCommand(
            ...
            'Replace items with default items',
            FCVAR_CHEAT
        )
        ...
        Convars:RegisterCommand(
            ...
            'Remove cosmetic in certain slot',
            FCVAR_CHEAT
        )
        ...
        Convars:RegisterCommand(
            ...
            'Remove all cosmetics from hero',
            FCVAR_CHEAT
        )
    end
end
...
function CosmeticLib:_CreateTables()
    ...
    for CosmeticID, CosmeticTable in pairs(CosmeticLib._AllItemsByID) do -- Extract only from items block
        if CosmeticTable['prefab'] then
            if
                CosmeticTable['prefab'] == 'default_item' and CosmeticTable['used_by_heroes'] and
                    type(CosmeticTable['used_by_heroes']) == 'table'
             then -- Insert default items
                CosmeticLib:_InsertIntoDefaultTable(CosmeticID)

                CosmeticLib._NameToID[CosmeticTable['name']] = CosmeticID
            elseif
                CosmeticTable['prefab'] == 'wearable' and CosmeticTable['used_by_heroes'] and
                    type(CosmeticTable['used_by_heroes']) == 'table'
             then -- Insert wearable items
    ...
    for CosmeticID, CosmeticTable in pairs(CosmeticLib._AllItemsByID) do -- Extract only from items block
        if
            CosmeticTable['prefab'] and CosmeticTable['prefab'] == 'bundle' and CosmeticTable['used_by_heroes'] ~= nil and
                type(CosmeticTable['used_by_heroes']) == 'table'
         then
...
function CosmeticLib:_InsertIntoCosmeticTable(CosmeticID, table_to_insert)
    ...
    if not selected_item['used_by_heroes'] or not selected_item['model_player'] then
        return
    end
...
function CosmeticLib:GetAllAvailableWearablesName()
    if CosmeticLib._NameToID then
        ...
        print(
            '[CosmeticLib:Getter] Error: No cosmetic table found. Please verify that you have item_games.txt in your vpk'
        )
...
function CosmeticLib:EquipSet(unit, hero_name, set_id)
    if unit and hero_name and set_id and CosmeticLib:_Identify(unit) then
        ...
        if
            selected_item and CosmeticLib._SetByHeroes[hero_name] and
                CosmeticLib._SetByHeroes[hero_name][selected_item['name']]
         then

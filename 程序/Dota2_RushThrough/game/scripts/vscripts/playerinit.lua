...
function createunit(BlockIndex)
    ...
    for i = 1, AnmNumArr[BlockIndex] do
        ...
        location =
            Vector(
            math.random(500) + PosArray[BlockIndex].x - 250,
            math.random(250) + PosArray[BlockIndex].y - 250,
            PosArray[BlockIndex].z
        )
...
function createBoss(BlockIndex)
    local unit =
        CreateUnitByName(AnmArr[BlockIndex] .. 'Boss', PosArray[BlockIndex], true, nil, nil, DOTA_TEAM_NEUTRALS)
    unit:SetContext('name', AnmArr[BlockIndex], 0)
end
